...
const Viewapplication = () => {
  ...
  const getapplication = async () => {
    try {
      ...
      setdata(res.data);
      setFilteredData(res.data); // set initially
      setloading(true);
    } catch (error) {
      console.log(error);
    }
    ...
    if (!searchTerm.trim()) {
      ...
      const results = data.filter(
        (item) =>
          ...
          item.Intrestedcountry?.toLowerCase().includes(term)
      );
  ...
  return (
    <div className="flex flex-col dm-sans">
      {Viewapplicationmodal && (
        <Applicationmodal
          setviewapplicationmodal={setviewapplicationmodal}
          applicationID={getapplicationid}
        />
      )}
      {createapplicationmodal && (
        <Addapplicationmodal
          setcreateapplicationmodal={setcreateapplicationmodal}
          applicationID={getapplicationid}
          Email={selectedEmail}
 ...
 rounded-lg  w-full max-w-lg p-6"
            >
      ...
      <div className="flex flex-row justify-end mt-10">
        <div
          className="flex w-auto p-3 h-11 mr-[388px] rounded-lg gap-5 items-center justify-center bg-white/10 backdrop-blur-xl hover:bg-white/15 border-white/20 shadow-xl
  flex-row"
        >
        ...
        <div className="w-96 relative">
          ...
          <input
            ...
            className="block w-full p-2.5 ps-10 text-sm text-gray-900 rounded-lg bg-white/10 backdrop-blur-xl hover:bg-white/15 border-white/20 shadow-xl"
      ...
      <div className="w-[1100px] rounded ml-56 mb-10 mt-3 overflow-x-auto">
        <div className="relative w-full ">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead
              className="text-xs text-white uppercase bg-white/10 backdrop-blur-xl hover:bg-white/15 border-white/20 shadow-xl"
            >
              <tr>
                ...
                {filteredData.map((item) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        disabled={item.Movetovisa === false}
                        onClick={() => movetovisa(item.id)}
                        className="bg-green-500/30 p-2 rounded-md ml-2 text-white cursor-not-allowed backdrop-blur-md border border-green-300/40 shadow-inner"
                      >
                        {item.Movetovisa == false
                          ? "Moved to visa"
                          : "Move to visa"}
                      </button>
                      <button
                      ...
                      <div className="flex mt-[-20px] flex-row gap-2">
                        ...
                        <div
                          onClick={() => {
                            ...
                            setapplicationid(item.id);
                            setSelectedEmail(item.Emailaddress);
                            setSelectedCountry(item.Intrestedcountry);
                          }}
