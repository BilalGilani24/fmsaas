...
const academic = async () => {
  try {
    ...
    const [res, proficencyRes, workexpRes, suggestedRes,applicationcount,visalength] = await Promise.all([
      ...
      axios.post('/api/student/application', { userId: userId }),
  ...
  return (
    <div className="flex flex-col dm-sans gap-10">
      ...
      <div className="flex flex-row ml-36 mt-[-40px] justify-between items-center">
        <h1 className="text-2xl mt-10 dm-sans ml-64">Personal/Academic Details</h1>
      ...
      <div className="flex flex-row ml-96 mt-[-30px] items-center gap-3">
        {sections.map((section) => (
          <div
            key={section.id}
            className="w-64 cursor-pointer bg-white/10 backdrop-blur-xl hover:bg-white/15 border-white/20 shadow-xl
   rounded-md p-9 space-y-3 relative overflow-hidden"
          >
            ...
            <p className="text-sm text-white text-left  leading-6">
              {section.description}
            </p>
          </div>
        ))}
      ...
      <div className="flex flex-row ml-96 mt-[-20px] gap-3 mb-10">
        {details.map((detail) => (
          <div
            key={detail.id}
            className="w-[345px] cursor-pointer bg-white/10 backdrop-blur-xl hover:bg-white/15 border-white/20 shadow-xl
   rounded-md p-9 space-y-3 relative overflow-hidden"
          >
            <div className="w-24 h-24 bg-red-500 rounded-full absolute -right-5 -top-7">
              <p className="absolute bottom-6 left-7 text-white text-2xl">
