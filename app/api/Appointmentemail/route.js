...
export async function POST(req) {
  try {
    const { email, appointmentTime, appoinmentremarks } = await req.json();

    if (!email || !appointmentTime) {
      return new Response(
        JSON.stringify({ error: "Email and appointment time are required." }),
        { status: 400 }
      );
    }
  ...
  <div style="margin-bottom: 25px;">
    <p>Dear Student,</p>
    <p>Your appointment is scheduled at <b>${appointmentTime}</b>.</p>
    <p>Appointment Remarks: ${appoinmentremarks || ""}</p>
  ...
  <table style="width: 100%; border-collapse: collapse;">
    ...
    return new Response(
      JSON.stringify({ message: "Appointment email sent successfully!" }),
      { status: 200 }
    );
  } catch (error) {
